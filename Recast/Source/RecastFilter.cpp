  ...
  for (int z = 0; z < zSize; ++z) {
    for (int x = 0; x < xSize; ++x) {
      ...
      for (rcSpan *span = heightfield.spans[x + z * xSize]; span != nullptr; previousSpan = span, span = span->next) {
        ...
        if (!walkable && previousWasWalkable && static_cast<int>(span->smax) - static_cast<int>(previousSpan->smax) <= walkableClimb) {
  ...
  for (int z = 0; z < zSize; ++z) {
    for (int x = 0; x < xSize; ++x) {
      for (rcSpan *span = heightfield.spans[x + z * xSize]; span; span = span->next) {
        ...
        const int floor = static_cast<int>(span->smax);
        const int ceiling = span->next ? static_cast<int>(span->next->smin) : MAX_HEIGHTFIELD_HEIGHT;
        ...
        for (int direction = 0; direction < 4; ++direction) {
          ...
          int neighborCeiling = neighborSpan ? static_cast<int>(neighborSpan->smin) : MAX_HEIGHTFIELD_HEIGHT;
          ...
          for (; neighborSpan != nullptr; neighborSpan = neighborSpan->next) {
            const int neighborFloor = static_cast<int>(neighborSpan->smax);
            neighborCeiling = neighborSpan->next ? static_cast<int>(neighborSpan->next->smin) : MAX_HEIGHTFIELD_HEIGHT;
