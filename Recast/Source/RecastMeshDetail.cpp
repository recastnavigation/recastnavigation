...
static float distToPoly(const int nvert, const float* verts, const float* p)
{
    float dmin = FLT_MAX;
    int i, j, c = 0;
...
static int findEdge(const int* edges, const int nedges, const int s, const int t)
{
...
static int addEdge(rcContext* ctx, int* edges, int& nedges, const int maxEdges, const int s, const int t, const int l,
                   const int r)
{
    if (nedges >= maxEdges)
    {
...
static void updateLeftFace(int* e, const int s, const int t, const int f)
{
...
static bool overlapEdges(const float* pts, const int* edges, const int nedges, const int s1, const int t1)
{
    for (int i = 0; i < nedges; ++i)
    {
...
static void completeFacet(rcContext* ctx, const float* pts, const int npts, int* edges, int& nedges, const int maxEdges,
                          int& nfaces, int e)
{
    ...
    if (sampleDist > 0)
    {
        for (int i = 0, j = nin - 1; i < nin; j = i++)
        {
            ...
            for (int k = 0; k <= nn; ++k)
            {
                ...
                pos[1] = vj[1] + dy * u;
                pos[2] = vj[2] + dz * u;
                pos[1] = static_cast<float>(getHeight(pos[0], pos[1], pos[2], cs, ics, chf.ch, heightSearchRadius, hp))
                    * chf.ch;
            }
            // Simplify samples.
...
static void push3(rcIntArray& queue, const int v1, const int v2, const int v3)
{
                          ...
                          const int region)
{
    ...
    while (head * 3 < queue.size())
    {
        ...
        for (int dir = 0; dir < 4; ++dir)
        {
            ...
            if (static_cast<unsigned int>(hx) >= static_cast<unsigned int>(hp.width) || static_cast<unsigned int>(hy) >=
                static_cast<unsigned int>(hp.height))
                continue;
...
bool rcMergePolyMeshDetails(rcContext* ctx, rcPolyMeshDetail** meshes, const int nmeshes, rcPolyMeshDetail& mesh)
{
    ...
    for (int i = 0; i < nmeshes; ++i)
    {
        ...
        for (int j = 0; j < dm->nmeshes; ++j)
        {
            ...
            const unsigned int* src = &dm->meshes[j * 4];
