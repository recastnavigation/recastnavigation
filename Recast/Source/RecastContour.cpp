...
static int getCornerHeight(const int x, const int y, const int i, const int dir,
...
static bool mergeContours(rcContour& ca, rcContour& cb, const int ia, const int ib)
{
	...
	const auto verts = static_cast<int*>(rcAlloc(sizeof(int) * maxVerts * 4, RC_ALLOC_PERM));
	if (!verts)
		return false;
...
static void mergeRegionHoles(rcContext* ctx, const rcContourRegion& region)
{
	...
	rcScopedDelete<rcPotentialDiagonal> diags(static_cast<rcPotentialDiagonal*>(rcAlloc(sizeof(rcPotentialDiagonal) * maxVerts, RC_ALLOC_TEMP)));
	if (!diags)
	{
					 ...
					 rcContourSet& cset, const int buildFlags)
{
	...
	rcScopedDelete<unsigned char> flags(static_cast<unsigned char*>(rcAlloc(sizeof(unsigned char) * chf.spanCount, RC_ALLOC_TEMP)));
	if (!flags)
	{
			...
			for (int i = static_cast<int>(c.index), ni = static_cast<int>(c.index + c.count); i < ni; ++i)
			{
				...
				if (simplified.size()/4 >= 3)
				{
					if (cset.nconts >= maxContours)
					{
						...
						maxContours *= 2;
						const auto newConts = static_cast<rcContour*>(rcAlloc(sizeof(rcContour) * maxContours, RC_ALLOC_PERM));
	...
	if (cset.nconts > 0)
	{
		...
		rcScopedDelete<signed char> winding(static_cast<signed char*>(rcAlloc(sizeof(signed char) * cset.nconts, RC_ALLOC_TEMP)));
		if (!winding)
		{
		...
		if (nholes > 0)
		{
			...
			rcScopedDelete<rcContourRegion> regions(static_cast<rcContourRegion*>(rcAlloc(sizeof(rcContourRegion) * nregions, RC_ALLOC_TEMP)));
			if (!regions)
			{
      ...
      for (int i = static_cast<int>(c.index), ni = static_cast<int>(c.index + c.count); i < ni; ++i)
      {
        ...
        if (simplified.size()/4 >= 3)
        {
          if (cset.nconts >= maxContours)
          {
            ...
            const int oldMax = maxContours;
            maxContours *= 2;
            auto* newConts = static_cast<rcContour*>(rcAlloc(sizeof(rcContour) * maxContours, RC_ALLOC_PERM));
            for (int j = 0; j < cset.nconts; ++j)
            {
              newConts[j] = cset.conts[j];
              // Reset source pointers to prevent data deletion.
          ...
          cont->reg = reg;
          cont->area = area;
          cont->size = static_cast<float>(chf.sizes[reg]) * chf.cs;
        }
      }
    }
  }
