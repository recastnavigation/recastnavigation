...
ConvexVolumeTool::ConvexVolumeTool() : m_sample(nullptr),
                                       m_areaType(SAMPLE_POLYAREA_GRASS),
                                       m_polyOffset(0.0f),
                                       m_boxHeight(6.0f),
                                       m_boxDescent(1.0f), m_pts{},
                                       m_npts(0), m_hull{},
                                       m_nhull(0) {
}
...
void ConvexVolumeTool::handleClick(const float * /*s*/, const float *p, const bool shift) {
  if (!m_sample)
    return;
  ...
  if (shift) {
    ...
    if (m_npts && rcVdistSqr(p, &m_pts[(m_npts - 1) * 3]) < rcSqr(0.2f)) {
      if (m_nhull > 2) {
        ...
        minh -= m_boxDescent;
        const float maxh = minh + m_boxHeight;
