...
void InputGeom::drawConvexVolumes(struct duDebugDraw* dd, bool /*hilight*/)
{
	...
	for (int i = 0; i < m_volumeCount; ++i)
	{
		...
		unsigned int col = duTransCol(sampleAreaToCol(vol->area), 32);
	...
	{
		const ConvexVolume* vol = &m_volumes[i];
		unsigned int col = duTransCol(sampleAreaToCol(vol->area), 220);
		for (int j = 0, k = vol->nverts-1; j < vol->nverts; k = j++)
		{
			const float* va = &vol->verts[k*3];
			const float* vb = &vol->verts[j*3];
	...
	{
		const ConvexVolume* vol = &m_volumes[i];
		unsigned int col = duDarkenCol(duTransCol(sampleAreaToCol(vol->area), 220));
		for (int j = 0; j < vol->nverts; ++j)
		{
