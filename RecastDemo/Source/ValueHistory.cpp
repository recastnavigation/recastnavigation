...
void drawGraphBackground(const GraphParams* p)
{
	...
	for (int i = 0; i <= p->ndiv; ++i)
	{
		...
		// Clear the stream.
		text.str(std::string());
		text << std::fixed << std::setprecision(2) << v << " " << p->units;

		const float fy = oy + v * sy;
		imguiDrawText(p->x + p->w - p->pad, (int)fy - 4, IMGUI_ALIGN_RIGHT, text.str().c_str(), imguiRGBA(0, 0, 0, 255));
...
void drawGraph(const GraphParams* p, const ValueHistory* graph, int idx, const char* label, const unsigned int col)
{
	...
	std::stringstream text;
	text << std::fixed << std::setprecision(2) << graph->getAverage() << " " << p->units;
	imguiDrawText(ix + size + 5, iy + 3, IMGUI_ALIGN_LEFT, label, imguiRGBA(255, 255, 255, 192));
	imguiDrawText(ix + size + 150, iy + 3, IMGUI_ALIGN_RIGHT, text.str().c_str(), imguiRGBA(255, 255, 255, 128));
}
