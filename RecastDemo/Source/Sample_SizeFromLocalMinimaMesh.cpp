...
Sample_SizeFromLocalMinimaMesh::Sample_SizeFromLocalMinimaMesh() :
...
void Sample_SizeFromLocalMinimaMesh::cleanup()
{
	delete [] m_triareas;
	m_triareas = nullptr;
...
void Sample_SizeFromLocalMinimaMesh::handleSettings()
{
...
void Sample_SizeFromLocalMinimaMesh::handleTools()
{
	const int type = !m_tool ? TOOL_NONE : m_tool->type();
	
	if (imguiCheck("Test Navmesh", type == TOOL_NAVMESH_TESTER))
	{
		setTool(new UnitedSizeNavMeshTesterTool);
	}
...
void Sample_SizeFromLocalMinimaMesh::handleDebugMode()
{
	...
	if (m_geom)
	{
		...
		valid[DRAWMODE_COMPACT_REGIONS] = m_chf != nullptr;
                valid[DRAWMODE_COMPACT_REGION_DISTANCE] = m_chf != nullptr;
	...
	if (imguiCheck("Compact Regions", m_drawMode == DRAWMODE_COMPACT_REGIONS, valid[DRAWMODE_COMPACT_REGIONS]))
        ...
        if (imguiCheck("Compact Region Distance", m_drawMode == DRAWMODE_COMPACT_REGION_DISTANCE, valid[DRAWMODE_COMPACT_REGION_DISTANCE]))
                m_drawMode = DRAWMODE_COMPACT_REGION_DISTANCE;
...
void Sample_SizeFromLocalMinimaMesh::handleRender()
{
	...
	if (m_chf && m_drawMode == DRAWMODE_COMPACT_REGIONS)
        ...
        if (m_chf && m_drawMode == DRAWMODE_COMPACT_REGION_DISTANCE)
                duDebugDrawCompactHeightfieldRegionDistance(&m_dd, *m_chf);
...
void Sample_SizeFromLocalMinimaMesh::handleRenderOverlay(double* proj, double* model, int* view)
{
	if (m_tool)
...
void Sample_SizeFromLocalMinimaMesh::handleMeshChanged(InputGeom* geom)
{
...
bool Sample_SizeFromLocalMinimaMesh::handleBuild()
{
	if (!m_geom || !m_geom->getMesh())
	{
    ...
    //
    // Step 5. Extract Distance in regions
    //
    if ( !rcCalculateDistancePerRegion( m_ctx, *m_chf ))
    {
      m_ctx->log(RC_LOG_ERROR, "buildNavigation: Could not extract Distances per region");
    }
	...
	//
	// (Optional) Step 9. Create Detour data from Recast poly mesh.
	//
	...
	if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
	{
		...
		if (!dtCreateNavMeshDataWithSize(&params, &navData, &navDataSize))
		{
