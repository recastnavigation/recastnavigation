...
bool Sample_TempObstacles::handleBuild()
{
	...
	rcCalcGridSize(bmin, bmax, m_cellSize, &gw, &gh);
    // 计算tile长宽数量
	const int ts = (int)m_tileSize; // tile 大小
	const int tw = (gw + ts-1) / ts; // w 方向的tile数量
	const int th = (gh + ts-1) / ts; // h 方向的tile数量

	// Generation params.
	rcConfig cfg;
	memset(&cfg, 0, sizeof(cfg));
	cfg.cs = m_cellSize; // 格子大涉
	cfg.ch = m_cellHeight; // 格了高度
	cfg.walkableSlopeAngle = m_agentMaxSlope; // agent 行走倾斜角度
	cfg.walkableHeight = (int)ceilf(m_agentHeight / cfg.ch); // agent 高度格子数
	cfg.walkableClimb = (int)floorf(m_agentMaxClimb / cfg.ch); // agent 可跨越高度格子数
	cfg.walkableRadius = (int)ceilf(m_agentRadius / cfg.cs); // agent 半径格子数
	cfg.maxEdgeLen = (int)(m_edgeMaxLen / m_cellSize); // 边界最长格子数
	cfg.maxSimplificationError = m_edgeMaxError; // 简化边界时边界长度
	cfg.minRegionArea = (int)rcSqr(m_regionMinSize);		// Note: area = size*size
	cfg.mergeRegionArea = (int)rcSqr(m_regionMergeSize);	// Note: area = size*size
	cfg.maxVertsPerPoly = (int)m_vertsPerPoly; // 每个多边形顶点数量
	cfg.tileSize = (int)m_tileSize; // tile 大小
	cfg.borderSize = cfg.walkableRadius + 3; // Reserve enough padding.
	cfg.width = cfg.tileSize + cfg.borderSize*2; // w
	cfg.height = cfg.tileSize + cfg.borderSize*2; // h
	cfg.detailSampleDist = m_detailSampleDist < 0.9f ? 0 : m_cellSize * m_detailSampleDist; // 细分多边形时取样高度
	cfg.detailSampleMaxError = m_cellHeight * m_detailSampleMaxError; // 细分多边形时取样边界
	rcVcopy(cfg.bmin, bmin); // 几何盒最小角点
	rcVcopy(cfg.bmax, bmax); // 几何盒最大角点
	
	// Tile cache params.
	dtTileCacheParams tcparams;
	memset(&tcparams, 0, sizeof(tcparams));
	rcVcopy(tcparams.orig, bmin); // 以最小角点作为原点
	tcparams.cs = m_cellSize; // 格子大小
	tcparams.ch = m_cellHeight; // 格子高度
	tcparams.width = (int)m_tileSize; // tile w
	tcparams.height = (int)m_tileSize; // tile h
	tcparams.walkableHeight = m_agentHeight; // agent 高度
	tcparams.walkableRadius = m_agentRadius; // agent 半径
	tcparams.walkableClimb = m_agentMaxClimb;// agent 可跨越高度
	tcparams.maxSimplificationError = m_edgeMaxError; // 细分多边形时取样边界
	tcparams.maxTiles = tw*th*EXPECTED_LAYERS_PER_TILE; // 最大 tile 数量
	tcparams.maxObstacles = 128; // 最大阻挡数量
