...
int main(int argc, char * argv[]) {
    ...
    while (!done) {
        ...
        if (processHitTest && geom && sample) {
            if (float hitTime; geom->raycastMesh(rayStart, rayEnd, hitTime)) {
        ...
        if (constexpr float MIN_FRAME_TIME = 1.0f / 40.0f; dt < MIN_FRAME_TIME) {
        ...
        moveFront = rcClamp(
            moveFront + dt * 4.f * ((keystate[SDL_SCANCODE_W] || keystate[SDL_SCANCODE_UP]) ? 1.f : -1.f), 0.0f, 1.0f);
        moveLeft = rcClamp(
            moveLeft + dt * 4.f * ((keystate[SDL_SCANCODE_A] || keystate[SDL_SCANCODE_LEFT]) ? 1.f : -1.f), 0.0f, 1.0f);
        moveBack = rcClamp(
            moveBack + dt * 4.f * ((keystate[SDL_SCANCODE_S] || keystate[SDL_SCANCODE_DOWN]) ? 1.f : -1.f), 0.0f, 1.0f);
        moveRight = rcClamp(
            moveRight + dt * 4.f * ((keystate[SDL_SCANCODE_D] || keystate[SDL_SCANCODE_RIGHT]) ? 1.f : -1.f), 0.0f,
            1.0f);
        moveUp = rcClamp(moveUp + dt * 4.f * ((keystate[SDL_SCANCODE_Q] || keystate[SDL_SCANCODE_PAGEUP]) ? 1.f : -1.f),
                         0.0f, 1.0f);
        moveDown = rcClamp(
            moveDown + dt * 4.f * ((keystate[SDL_SCANCODE_E] || keystate[SDL_SCANCODE_PAGEDOWN]) ? 1.f : -1.f), 0.0f,
            1.0f);
        ...
        if (showSample) {
            ...
            if (imguiBeginScrollArea("Choose Sample", width - 10 - 250 - 10 - 200, height - 10 - 250, 200, 250,
                                     &levelScroll))
                mouseOverMenu = true;

            Sample *newSample = nullptr;
            for (auto &[create, name]: g_samples) {
                if (imguiItem(name.c_str())) {
                    newSample = create();
                    if (newSample)
                        sampleName = name;
                }
        ...
        if (showLevels) {
            static int levelScroll = 0;
            if (imguiBeginScrollArea("Choose Level", width - 10 - 250 - 10 - 200, height - 10 - 450, 200, 450,
                                     &levelScroll))
                mouseOverMenu = true;
