...
class NavMeshTileToolLCM : public SampleTool
{
	Sample_TileMeshLCM* m_sample;
	float m_hitPos[3];
	bool m_hitPosSet;
...
public:
	...
	virtual ~NavMeshTileToolLCM();

	virtual int type() { return TOOL_TILE_EDIT; }

	virtual void init(Sample* sample)
	{
		m_sample = (Sample_TileMeshLCM*)sample;
	}
...
NavMeshTileToolLCM::~NavMeshTileToolLCM()
{
	// Defined out of line to fix the weak v-tables warning
}

Sample_TileMeshLCM::Sample_TileMeshLCM() :
	...
	m_tileTriCount(0)
{
	...
	setTool(new NavMeshTileToolLCM);
}
...
void Sample_TileMeshLCM::cleanup()
{
	delete [] m_triareas;
	m_triareas = 0;
...
void Sample_TileMeshLCM::handleSettings()
{
...
void Sample_TileMeshLCM::handleTools()
{
...
void Sample_TileMeshLCM::handleDebugMode()
{
...
void Sample_TileMeshLCM::handleRender()
{
...
void Sample_TileMeshLCM::handleRenderOverlay(double* proj, double* model, int* view)
{
	GLdouble x, y, z;
...
void Sample_TileMeshLCM::handleMeshChanged(InputGeom* geom)
{
...
bool Sample_TileMeshLCM::handleBuild()
{
	if (!m_geom || !m_geom->getMesh())
	{
...
void Sample_TileMeshLCM::collectSettings(BuildSettings& settings)
{
...
void Sample_TileMeshLCM::buildTile(const float* pos)
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void Sample_TileMeshLCM::getTilePos(const float* pos, int& tx, int& ty)
{
	if (!m_geom) return;
...
void Sample_TileMeshLCM::removeTile(const float* pos)
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void Sample_TileMeshLCM::buildAllTiles()
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void Sample_TileMeshLCM::removeAllTiles()
{
	if (!m_geom || !m_navMesh)
		return;
...
unsigned char* Sample_TileMeshLCM::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
