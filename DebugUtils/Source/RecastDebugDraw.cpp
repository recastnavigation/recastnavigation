...
void duDebugDrawTriMesh(duDebugDraw* dd, const float* verts, int /*nverts*/,
                        const int* tris, const float* normals, const int ntris,
...
void duDebugDrawCompactHeightfieldRegions(duDebugDraw* dd, const rcCompactHeightfield& chf)
{
    ...
    for (int y = 0; y < chf.height; ++y)
    {
        for (int x = 0; x < chf.width; ++x)
        {
            ...
            {
                const rcCompactSpan& s = chf.spans[i];
                const float fy = chf.bmin[1] + static_cast<float>(s.y) * ch;
                unsigned int color;
                if (s.reg)
...
static void getContourCenter(const rcContour* cont, const float* orig, const float cs, const float ch, float* center)
{
    center[0] = 0;
    center[1] = 0;
    center[2] = 0;
...
static const rcContour* findContourFromSet(const rcContourSet& cset, const unsigned short reg)
{
...
void duDebugDrawRawContours(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
    ...
    {
        const rcContour& c = cset.conts[i];
        const unsigned int color = duDarkenCol(duIntToCol(c.reg, a));
...
void duDebugDrawContours(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
    ...
    for (int i = 0; i < cset.nconts; ++i)
    {
        ...
        for (int j = 0, k = c.nverts - 1; j < c.nverts; k = j++)
        {
            ...
            const unsigned int col = va[3] & RC_AREA_BORDER ? bcolor : color;
...
void duDebugDrawPolyMeshDetail(duDebugDraw* dd, const rcPolyMeshDetail& dmesh)
{
    ...
    for (int i = 0; i < dmesh.nmeshes; ++i)
    {
        ...
        const unsigned int color = duIntToCol(i, 192);
