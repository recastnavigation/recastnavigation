...
void duDebugDrawTriMesh(duDebugDraw *dd, const float *verts, int /*nverts*/, const int *tris, const float *normals, const int ntris, const uint8_t *flags, const float texScale) {
  if (!dd)
    return;
  if (!verts)
    return;
  ...
  for (int i = 0; i < ntris * 3; i += 3) {
    ...
    uint32_t color;
    const uint8_t a = static_cast<uint8_t>(220 * (2 + norm[0] + norm[1]) / 4);
    if (flags && !flags[i / 3])
  ...
  for (int y = 0; y < chf.height; ++y) {
    for (int x = 0; x < chf.width; ++x) {
      ...
      for (unsigned i = c.index, ni = c.index + c.count; i < ni; ++i) {
        const rcCompactSpan &s = chf.spans[i];
        const float fy = chf.bmin[1] + (s.y + 1) * ch;
        const uint8_t cd = static_cast<uint8_t>(chf.dist[i] * dscale);
...
void duDebugDrawRegionConnections(duDebugDraw *dd, const rcContourSet &cset, const float alpha) {
  ...
  for (int i = 0; i < cset.nconts; ++i) {
    ...
    for (int j = 0; j < cont->nverts; ++j) {
      ...
      if (v[3] == 0 || static_cast<uint16_t>(v[3]) < cont->reg)
        continue;
      if (const rcContour *cont2 = findContourFromSet(cset, static_cast<uint16_t>(v[3]))) {
        float pos2[3];
...
void duDebugDrawRawContours(duDebugDraw *dd, const rcContourSet &cset, const float alpha) {
  ...
  for (int i = 0; i < cset.nconts; ++i) {
    const rcContour &c = cset.conts[i];
    const uint32_t color = duDarkenCol(duIntToCol(c.reg, a));
