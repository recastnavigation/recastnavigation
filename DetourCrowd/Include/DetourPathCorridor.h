...
public:
  ...
  int findCorners(float *cornerVerts, unsigned char *cornerFlags,dtPolyRef *cornerPolys, int maxCorners,const dtNavMeshQuery *navquery, const dtQueryFilter *filter) const;
  ...
  ///  @param[in]		navquery				The query object used to build the corridor.
  ///  @param[in]		filter					The filter to apply to the operation.
  void optimizePathVisibility(const float *next, float pathOptimizationRange,const dtNavMeshQuery *navquery, const dtQueryFilter *filter);
  ...
  bool moveOverOffmeshConnection(dtPolyRef offMeshConRef, dtPolyRef *refs,float *startPos, float *endPos,const dtNavMeshQuery *navquery);

  bool fixPathStart(dtPolyRef safeRef, const float *safePos);

  bool trimInvalidPath(dtPolyRef safeRef, const float *safePos,const dtNavMeshQuery *navquery, const dtQueryFilter *filter);
...
int dtMergeCorridorStartMoved(dtPolyRef *path, int npath, int maxPath,const dtPolyRef *visited, int nvisited);

int dtMergeCorridorEndMoved(dtPolyRef *path, int npath, int maxPath,const dtPolyRef *visited, int nvisited);

int dtMergeCorridorStartShortcut(dtPolyRef *path, int npath, int maxPath,const dtPolyRef *visited, int nvisited);
