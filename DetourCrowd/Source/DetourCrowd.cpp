...
constexpr int MAX_PATHQUEUE_NODES = 4096;
constexpr int MAX_COMMON_NODES = 512;
...
int getNeighbours(const float *pos, const float height, const float range,
                         ...
                         dtCrowdAgent **agents, const int /*nagents*/, const dtProximityGrid *grid) {
  int n = 0;

  static constexpr int MAX_NEIS = 32;
  uint16_t ids[MAX_NEIS];
...
int dtCrowd::addAgent(const float *pos, const dtCrowdAgentParams *params) const {
  ...
  dtPolyRef ref = 0;
  dtVcopy(nearest, pos);
  if (dtStatusFailed(m_navquery->findNearestPoly(pos, m_agentPlacementHalfExtents, &m_filters[ag->params.queryFilterType], &ref, nearest))) {
    dtVcopy(nearest, pos);
    ref = 0;
  }
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo *debug) {
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    m_grid->addItem(static_cast<uint16_t>(i), p[0] - r, p[2] - r, p[0] + r, p[2] + r);
  }
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    if (dtVdist2DSqr(ag->npos, ag->boundary.getCenter()) > dtSqr(ag->params.collisionQueryRange * 0.25f) ||
  ...
  for (int i = 0; i < nagents; ++i) {
    ...
    // Check
    if (overOffmeshConnection(ag, ag->params.radius * 2.25f)) {
