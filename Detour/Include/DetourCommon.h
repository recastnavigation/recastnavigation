...
template <class T>
T dtClamp(T v, T mn, T mx) { return v < mn ? mn : v > mx ? mx
                                                         : v; }
...
inline void dtVnormalize(float *v) {
  const float d = 1.0f / dtMathSqrtf(dtSqr(v[0]) + dtSqr(v[1]) + dtSqr(v[2]));
  v[0] *= d;
  v[1] *= d;
  v[2] *= d;
}
...
inline bool dtVisfinite2D(const float *v) {
  const bool result = dtMathIsfinite(v[0]) && dtMathIsfinite(v[2]);
  return result;
}
...
inline int dtAlign4(const int x) { return x + 3 & ~3; }
...
void dtRandomPointInConvexPoly(const float *pts, int npts, float *areas, float s, float t, float *out);

template <typename TypeToRetrieveAs>
TypeToRetrieveAs *dtGetThenAdvanceBufferPointer(const unsigned char *&buffer, const std::size_t distanceToAdvance) {
  auto *returnPointer = reinterpret_cast<TypeToRetrieveAs *>(buffer);
  buffer += distanceToAdvance;
  return returnPointer;
}

template <typename TypeToRetrieveAs>
TypeToRetrieveAs *dtGetThenAdvanceBufferPointer(unsigned char *&buffer, const std::size_t distanceToAdvance) {
  auto *returnPointer = reinterpret_cast<TypeToRetrieveAs *>(buffer);
  buffer += distanceToAdvance;
  return returnPointer;
}
