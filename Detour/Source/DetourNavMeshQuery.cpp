...
dtStatus dtNavMeshQuery::_appendVertex(const float *pos, const float *shifted_pos, const unsigned char flags, const dtPolyRef ref,
									   float *straightPath, float *_straightPath, unsigned char *straightPathFlags, dtPolyRef *straightPathRefs,
									   int *straightPathCount, const int maxStraightPath) const
{
	...
	{
		// Append new vertex.
		dtVcopy(&straightPath[(*straightPathCount) * 3], shifted_pos);
		dtVcopy(&_straightPath[(*straightPathCount) * 3], pos);
...
// MODIFIED ====================================================
dtStatus dtNavMeshQuery::_appendPortals(const int startIdx, const int endIdx, const float *endPos, const dtPolyRef *path,
										float *straightPath, float *_straightPath, unsigned char *straightPathFlags, dtPolyRef *straightPathRefs,
										int *straightPathCount, const int maxStraightPath, const int options) const
{
	const float *startPos = &straightPath[(*straightPathCount - 1) * 3];
	...
	for (int i = startIdx; i < endIdx; i++)
	{
		...
		if (dtIntersectSegSeg2D(startPos, endPos, left, right, s, t))
		{
			...
			stat = _appendVertex(pt, pt, 0, path[i + 1],
								 straightPath, _straightPath, straightPathFlags, straightPathRefs,
								 straightPathCount, maxStraightPath);
			if (stat != DT_IN_PROGRESS)
				return stat;
		}
	...
	*straightPathCount = 0;
	...
	if (pathSize > 1)
	{
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (i + 1 < pathSize) // if it's not the last poly in path
			{
...
// MODIFIED STRING-PULLING MIDDLE POINT =========================================================
dtStatus dtNavMeshQuery::findStraightPath_shifted(const float *startPos, const float *endPos,
												  const dtPolyRef *path, const int pathSize, float *straightPath,
												  float *_straightPath, const float shift_coe, unsigned char *straightPathFlags, dtPolyRef *straightPathRefs,
												  int *straightPathCount, const int maxStraightPath, const int options) const
{
	...
	*straightPathCount = 0;
	...
	// Add start point.
	stat = _appendVertex(closestStartPos, closestStartPos, DT_STRAIGHTPATH_START, path[0],
						 straightPath, _straightPath, straightPathFlags, straightPathRefs,
						 straightPathCount, maxStraightPath);
	if (stat != DT_IN_PROGRESS)
		return stat;
	...
	if (pathSize > 1)
	{
		...
		const int STEP_LIMIT = 19;
		const int STEP_INIT_MIN = 0;
		const int STEP_INIT_MAX = 15;
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (i + 1 < pathSize) // if it's not the last poly in path
			{
				...
				if (dtStatusFailed(getPortalPoints(path[i], path[i + 1], left, right, fromType, toType)))
				{
					...
					// Ignore status return value as we're just about to return anyway.
					_appendVertex(closestEndPos, closestEndPos, 0, path[i],
								  straightPath, _straightPath, straightPathFlags, straightPathRefs,
								  straightPathCount, maxStraightPath);
			...
			if (dtTriArea2D(portalApex, portalRight, right) <= 0.0f)
			{
				if ((dtVequal(portalApex, portalRight) || dtTriArea2D(portalApex, portalLeft, right) > 0.0f) && step_count < STEP_LIMIT)
				// if ((dtVequal(portalApex, portalRight) || dtTriArea2D(portalApex, portalLeft, right) > 0.0f))
				...
				else
				{
					...
					if (!rightPolyRef)
						flags = DT_STRAIGHTPATH_END;
					else if (rightPolyRef == DT_POLYTYPE_OFFMESH_CONNECTION)
						flags = DT_STRAIGHTPATH_OFFMESH_CONNECTION;
					dtPolyRef ref = rightPolyRef;

					// Append or update vertex
					float shifted_apex[3];
					if (shift_coe != 0.0)
					{
						for (int i = 0; i < 3; i++)
						{
							shifted_apex[i] = portalLeft[i] + (portalRight[i] - portalLeft[i]) * shift_coe;
						}
					}
					else
					{
						dtVcopy(shifted_apex, portalLeft);
					}

					stat = _appendVertex(portalApex, shifted_apex, flags, ref,
										 straightPath, _straightPath, straightPathFlags, straightPathRefs,
										 straightPathCount, maxStraightPath);
					// stat = appendVertex(portalApex, flags, ref,
					// 					straightPath, straightPathFlags, straightPathRefs,
					...
					// Restart
					i = apexIndex;
					step_count = (rand() % (STEP_INIT_MAX - STEP_INIT_MIN + 1)) + STEP_INIT_MIN;
					continue;
				}
			}
			...
			if (dtTriArea2D(portalApex, portalLeft, left) >= 0.0f)
			{
				if ((dtVequal(portalApex, portalLeft) || dtTriArea2D(portalApex, portalRight, left) < 0.0f) && step_count < STEP_LIMIT)
				// if ((dtVequal(portalApex, portalLeft) || dtTriArea2D(portalApex, portalRight, left) < 0.0f))
				{
					dtVcopy(portalLeft, left);
					leftPolyRef = (i + 1 < pathSize) ? path[i + 1] : 0;
				...
				else
				{
					...
					if (!leftPolyRef)
						flags = DT_STRAIGHTPATH_END;
					else if (leftPolyRef == DT_POLYTYPE_OFFMESH_CONNECTION)
						flags = DT_STRAIGHTPATH_OFFMESH_CONNECTION;
					dtPolyRef ref = leftPolyRef;
					// Append or update vertex
					float shifted_apex[3];
					if (shift_coe != 0.0)
					{
						for (int i = 0; i < 3; i++)
						{
							shifted_apex[i] = portalRight[i] + (portalLeft[i] - portalRight[i]) * shift_coe;
						}
					}
					else
					{
	...
	// Ignore status return value as we're just about to return anyway.
	_appendVertex(closestEndPos, closestEndPos, DT_STRAIGHTPATH_END, 0,
				  straightPath, _straightPath, straightPathFlags, straightPathRefs,
				  straightPathCount, maxStraightPath);
	// appendVertex(closestEndPos, DT_STRAIGHTPATH_END, 0,
	// 			 straightPath, straightPathFlags, straightPathRefs,
...
// MODIFIED STRING-PULLING REVERSED LEFT&RIGHT =========================================================
...
dtStatus dtNavMeshQuery::findDistanceToWall_filter(dtPolyRef startRef, float *centerPos,
												   float *obstacle_pos, float obstacle_radius,
												   const float maxRadius, const dtQueryFilter *filter,
												   float *hitDist, float *hitPos, float *hitNormal) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	...
	while (!m_openList->empty())
	{
		...
		for (int i = 0, j = (int)bestPoly->vertCount - 1; i < (int)bestPoly->vertCount; j = i++)
		{
			...
			float hit[3];
			hit[0] = vj[0] + (vi[0] - vj[0]) * tseg;
			hit[1] = vj[1] + (vi[1] - vj[1]) * tseg;
			hit[2] = vj[2] + (vi[2] - vj[2]) * tseg;

			if (validate_arrive(hit, obstacle_pos, obstacle_radius))
			{
				continue;
			}
