...
dtNavMeshQuery::dtNavMeshQuery() : m_nav(nullptr),
                                   ...
                                   m_nodePool(nullptr),
                                   m_openList(nullptr) {
  std::memset(&m_query, 0, sizeof(dtQueryData));
}
...
dtStatus dtNavMeshQuery::findRandomPointAroundCircle(const dtPolyRef startRef, const float *centerPos, const float maxRadius,
                                                     const dtQueryFilter *filter, float (*frand)(),
                                                     dtPolyRef *randomRef, float *randomPt) const {
  dtAssert(m_nav);
  dtAssert(m_nodePool);
  dtAssert(m_openList);
  if (!m_nav || !m_nodePool || !m_openList)
    return DT_FAILURE | DT_INVALID_PARAM;
  ...
  while (!m_openList->empty()) {
    ...
    for (uint32_t i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next) {
      ...
      float tseg;
      if (dtDistancePtSegSqr2D(centerPos, va, vb, tseg) > radiusSqr)
        continue;
...
///
dtStatus dtNavMeshQuery::closestPointOnPoly(const dtPolyRef ref, const float *pos, float *closest, bool *posOverPoly) const {
  dtAssert(m_nav);
  if (!m_nav)
    return DT_FAILURE | DT_INVALID_PARAM;
...
///
dtStatus dtNavMeshQuery::closestPointOnPolyBoundary(const dtPolyRef ref, const float *pos, float *closest) const {
  dtAssert(m_nav);
  if (!m_nav)
    return DT_FAILURE | DT_INVALID_PARAM;
...
/// of the polygon.
///
dtStatus dtNavMeshQuery::getPolyHeight(const dtPolyRef ref, const float *pos, float *height) const {
  dtAssert(m_nav);
  if (!m_nav)
    return DT_FAILURE | DT_INVALID_PARAM;
...
class dtFindNearestPolyQuery final : public dtPolyQuery {
...
public:
  ...
  ~dtFindNearestPolyQuery() override;
  ...
  void process(const dtMeshTile *tile, dtPoly **polys, dtPolyRef *refs, const int count) override {
  ...
  if (tile->bvTree) {
    ...
    while (node < end) {
      ...
      if (isLeafNode && overlap) {
        const dtPolyRef ref = base | static_cast<dtPolyRef>(node->i);
...
class dtCollectPolysQuery final : public dtPolyQuery {
  dtPolyRef *m_polys;
...
public:
  ...
  void process(const dtMeshTile *tile, dtPoly **polys, dtPolyRef *refs, const int count) override {
    ...
    const int numLeft = m_maxPolys - m_numCollected;
    int toCopy = count;
    if (toCopy > numLeft) {
      m_overflow = true;
      toCopy = numLeft;
...
dtStatus dtNavMeshQuery::queryPolygons(const float *center, const float *halfExtents,
  ...
  const dtStatus status = queryPolygons(center, halfExtents, filter, &collector);
  if (dtStatusFailed(status))
    return status;
...
///
dtStatus dtNavMeshQuery::findPath(const dtPolyRef startRef, const dtPolyRef endRef,
  ...
  if (!pathCount || !m_nav || !m_nodePool || !m_openList)
    return DT_FAILURE | DT_INVALID_PARAM;
...
dtStatus dtNavMeshQuery::getPathToNode(const dtNode *endNode, dtPolyRef *path, int *pathCount, const int maxPath) const {
...
/// path query.
///
dtStatus dtNavMeshQuery::initSlicedFindPath(const dtPolyRef startRef, const dtPolyRef endRef,
                                            const float *startPos, const float *endPos,
  ...
  if (options & DT_FINDPATH_ANY_ANGLE) {
    ...
    const float agentRadius = tile->header->walkableRadius;
...
dtStatus dtNavMeshQuery::updateSlicedFindPath(const int maxIter, int *doneIters) {
  ...
  while (iter < maxIter && !m_openList->empty()) {
    ...
    const dtMeshTile *parentTile = nullptr;
    const dtPoly *parentPoly = nullptr;
    const dtNode *parentNode = nullptr;
    if (bestNode->pidx) {
    ...
    if (parentRef) {
      if (dtStatusFailed(m_nav->getTileAndPolyByRef(parentRef, &parentTile, &parentPoly)) || (grandpaRef && !m_nav->isValidPolyRef(grandpaRef))) {
        // The polygon has disappeared during the sliced query, fail.
        m_query.status = DT_FAILURE;
        if (doneIters)
          *doneIters = iter;
  ...
  *pathCount = 0;
  ...
  if (m_query.startRef == m_query.endRef) {
    ...
    do {
      ...
      prev = node;
      const int nextRay = node->flags & DT_NODE_PARENT_DETACHED;      // keep track of whether parent is not adjacent (i.e. due to raycast shortcut)
...
dtStatus dtNavMeshQuery::finalizeSlicedFindPathPartial(const dtPolyRef *existing, const int existingSize,
  ...
  *pathCount = 0;
  ...
  if (!existing || existingSize <= 0 || !path || maxPath <= 0)
    return DT_FAILURE | DT_INVALID_PARAM;
...
dtStatus dtNavMeshQuery::appendPortals(const int startIdx, const int endIdx, const float *endPos, const dtPolyRef *path,
  ...
  for (int i = startIdx; i < endIdx; i++) {
    ...
    if (dtIntersectSegSeg2D(startPos, endPos, left, right, s, t)) {
      ...
      dtStatus stat = appendVertex(pt, 0, path[i + 1],
...
dtStatus dtNavMeshQuery::findStraightPath(const float *startPos, const float *endPos,
  ...
  *straightPathCount = 0;
  ...
  // Add start point.
  dtStatus stat = appendVertex(closestStartPos, DT_STRAIGHTPATH_START, path[0],
                               straightPath, straightPathFlags, straightPathRefs,
                               straightPathCount, maxStraightPath);
  if (stat != DT_IN_PROGRESS)
    return stat;
    ...
    for (int i = 0; i < pathSize; ++i) {
      ...
      if (dtTriArea2D(portalApex, portalRight, right) <= 0.0f) {
        if (dtVequal(portalApex, portalRight) || dtTriArea2D(portalApex, portalLeft, right) > 0.0f) {
          ...
          const dtPolyRef ref = leftPolyRef;
      ...
      if (dtTriArea2D(portalApex, portalLeft, left) >= 0.0f) {
        if (dtVequal(portalApex, portalLeft) || dtTriArea2D(portalApex, portalRight, left) < 0.0f) {
          ...
          if (!rightPolyRef)
            ...
            flags = DT_STRAIGHTPATH_OFFMESH_CONNECTION;
          const dtPolyRef ref = rightPolyRef;
...
/// position.
///
dtStatus dtNavMeshQuery::moveAlongSurface(const dtPolyRef startRef, const float *startPos, const float *endPos,
                                          const dtQueryFilter *filter,
                                          float *resultPos, dtPolyRef *visited, int *visitedCount, const int maxVisitedSize) const {
  dtAssert(m_nav);
  dtAssert(m_tinyNodePool);

  if (!visitedCount || !m_nav || !m_tinyNodePool)
    return DT_FAILURE | DT_INVALID_PARAM;
  ...
  *visitedCount = 0;
  ...
  const float searchRadSqr = dtSqr(dtVdist(startPos, endPos) / 2.0f + 0.001f);
  ...
  while (nstack) {
    ...
    for (int i = 0, j = static_cast<int>(curPoly->vertCount) - 1; i < static_cast<int>(curPoly->vertCount); j = i++) {
      ...
      if (curPoly->neis[j] & DT_EXT_LINK) {
        ...
        const uint32_t idx = static_cast<uint32_t>(curPoly->neis[j] - 1);
      ...
      if (!nneis) {
        ...
        for (int k = 0; k < nneis; ++k) {
          ...
          const float *vi = &verts[i * 3];
          float tseg;
          if ( dtDistancePtSegSqr2D(searchPos, vj, vi, tseg) > searchRadSqr)
            continue;
...
dtStatus dtNavMeshQuery::getPortalPoints(const dtPolyRef from, const dtPolyRef to, float *left, float *right,
...
dtStatus dtNavMeshQuery::getPortalPoints(const dtPolyRef from, const dtPoly *fromPoly, const dtMeshTile *fromTile,
                                         const dtPolyRef to, const dtPoly *toPoly, const dtMeshTile *toTile,
...
dtStatus dtNavMeshQuery::getEdgeMidPoint(const dtPolyRef from, const dtPolyRef to, float *mid) const {
  float left[3], right[3];
  uint8_t fromType;
  uint8_t toType;
...
dtStatus dtNavMeshQuery::getEdgeMidPoint(const dtPolyRef from, const dtPoly *fromPoly, const dtMeshTile *fromTile,
                                         const dtPolyRef to, const dtPoly *toPoly, const dtMeshTile *toTile,
                                         float *mid) {
  float left[3];
  float right[3];
...
///
dtStatus dtNavMeshQuery::raycast(const dtPolyRef startRef, const float *startPos, const float *endPos,
                                 const dtQueryFilter *filter,
...
dtStatus dtNavMeshQuery::raycast(const dtPolyRef startRef, const float *startPos, const float *endPos,
  ...
  const dtMeshTile *nextTile = prevTile = tile;
  const dtPoly *nextPoly = prevPoly = poly;
  if (prevRef)
  ...
  while (curRef) {
    ...
    for (uint32_t i = poly->firstLink; i != DT_NULL_LINK; i = tile->links[i].next) {
      ...
      if (link->side == 0 || link->side == 4) {
        ...
        // Find Z intersection.
        const float z = startPos[2] + (endPos[2] - startPos[2]) * tmax;
        if (z >= lmin && z <= lmax) {
          nextRef = link->ref;
          break;
        }
        ...
        // Find X intersection.
        const float x = startPos[0] + (endPos[0] - startPos[0]) * tmax;
        if (x >= lmin && x <= lmax) {
          nextRef = link->ref;
          break;
        }
    ...
    if (options & DT_RAYCAST_USE_COSTS) {
      ...
      const float *e1 = &verts[segMax * 3];
      const float *e2 = &verts[(segMax + 1) % nv * 3];
      float eDir[3], diff[3];
      dtVsub(eDir, e2, e1);
      dtVsub(diff, curPos, e1);
      const float s = dtSqr(eDir[0]) > dtSqr(eDir[2]) ? diff[0] / eDir[0] : diff[2] / eDir[2];
...
/// filled to capacity.
///
dtStatus dtNavMeshQuery::findPolysAroundCircle(const dtPolyRef startRef, const float *centerPos, const float radius,
                                               const dtQueryFilter *filter,
  ...
  if (!resultCount || !m_nav || !m_nodePool || !m_openList)
    return DT_FAILURE | DT_INVALID_PARAM;
...
///
dtStatus dtNavMeshQuery::findPolysAroundShape(const dtPolyRef startRef, const float *verts, const int nverts,
                                              const dtQueryFilter *filter,
                                              dtPolyRef *resultRef, dtPolyRef *resultParent, float *resultCost,
                                              int *resultCount, const int maxResult) const {
  dtAssert(m_nav);
...
dtStatus dtNavMeshQuery::getPathFromDijkstraSearch(const dtPolyRef endRef, dtPolyRef *path, int *pathCount, const int maxPath) const {
...
/// be filled to capacity.
///
dtStatus dtNavMeshQuery::findLocalNeighbourhood(const dtPolyRef startRef, const float *centerPos, const float radius,
                                                const dtQueryFilter *filter,
  ...
  *resultCount = 0;
  ...
  while (nstack) {
    ...
    for (uint32_t i = curPoly->firstLink; i != DT_NULL_LINK; i = curTile->links[i].next) {
      ...
      bool overlap = false;
      for (int j = 0; j < n; ++j) {
        const dtPolyRef pastRef = resultRef[j];
...
namespace {
void insertInterval(dtSegInterval *ints, int &nints, const int maxInts, const short tmin, const short tmax, const dtPolyRef ref) {
  if (nints + 1 > maxInts)
    return;
  ...
  if (nints - idx)
    std::memmove(ints + idx + 1, ints + idx, sizeof(dtSegInterval) * (nints - idx));
  // Store
  ints[idx].ref = ref;
  ints[idx].tmin = tmin;
  ints[idx].tmax = tmax;
...
///
dtStatus dtNavMeshQuery::getPolyWallSegments(const dtPolyRef ref, const dtQueryFilter *filter,
  ...
  *segmentCount = 0;
  ...
  for (int i = 0, j = static_cast<int>(poly->vertCount) - 1; i < static_cast<int>(poly->vertCount); j = i++) {
    ...
    if (poly->neis[j] & DT_EXT_LINK) {
      ...
      dtPolyRef neiRef = 0;
      if (poly->neis[j]) {
        const uint32_t idx = static_cast<uint32_t>(poly->neis[j] - 1);
...
///
dtStatus dtNavMeshQuery::findDistanceToWall(const dtPolyRef startRef, const float *centerPos, const float maxRadius,
                                            const dtQueryFilter *filter,
  ...
  while (!m_openList->empty()) {
    ...
    for (int i = 0, j = static_cast<int>(bestPoly->vertCount) - 1; i < static_cast<int>(bestPoly->vertCount); j = i++) {
      ...
      if (bestPoly->neis[j] & DT_EXT_LINK) {
        ...
        // Internal edge
        const uint32_t idx = static_cast<uint32_t>(bestPoly->neis[j] - 1);
        if (filter->passFilter(m_nav->getPolyRefBase(bestTile) | idx, bestTile, &bestTile->polys[idx]))
          continue;
      }
      ...
      float tseg;
      const float distSqr = dtDistancePtSegSqr2D(centerPos, vj, vi, tseg);
...
bool dtNavMeshQuery::isValidPolyRef(const dtPolyRef ref, const dtQueryFilter *filter) const {
  const dtMeshTile *tile = nullptr;
  const dtPoly *poly = nullptr;
...
///
bool dtNavMeshQuery::isInClosedList(const dtPolyRef ref) const {
  if (!m_nodePool)
    return false;

  dtNode *nodes[DT_MAX_STATES_PER_NODE];
  const int n = m_nodePool->findNodes(ref, nodes, DT_MAX_STATES_PER_NODE);
