...
dtStatus dtNavMeshQuery::closestPointOnPolyBoundary(const dtPolyRef ref, const float* pos, float* closest) const
{
    ...
    const bool inside = dtDistancePtPolyEdgesSqr(pos, verts, nv, edged, edget);
    if (inside)
    {
    ...
    *visitedCount = 0;
    ...
    while (nstack)
    {
        ...
        for (int i = 0, j = static_cast<int>(curPoly->vertCount) - 1; i < static_cast<int>(curPoly->vertCount); j = i++)
        {
            ...
            {
                const auto idx = static_cast<unsigned int>(curPoly->neis[j] - 1);
    ...
    *segmentCount = 0;
    ...
    for (int i = 0, j = static_cast<int>(poly->vertCount) - 1; i < static_cast<int>(poly->vertCount); j = i++)
    {
        ...
        else
        {
            ...
            if (poly->neis[j])
            {
                const auto idx = static_cast<unsigned int>(poly->neis[j] - 1);
                                            ...
                                            float* hitDist, float* hitPos, float* hitNormal) const
{
    ...
    while (!m_openList->empty())
    {
             ...
             ++)
        {
            ...
            {
                // Internal edge
                const auto idx = static_cast<unsigned int>(bestPoly->neis[j] - 1);
