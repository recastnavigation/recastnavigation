...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	const int vertsSize = dtAlignForType<dtPoly>(sizeof(float)*3*totVertCount);
	const int polysSize = dtAlignForType<dtLink>(sizeof(dtPoly)*totPolyCount);
	const int linksSize = dtAlignForType<dtPolyDetail>(sizeof(dtLink)*maxLinkCount);
	const int detailMeshesSize = dtAlignForType<float>(sizeof(dtPolyDetail)*params->polyCount);
	const int detailVertsSize = dtAlignForType<unsigned char>(sizeof(float)*3*uniqueDetailVertCount);
	const int detailTrisSize = dtAlignForType<dtBVNode>(sizeof(unsigned char)*4*detailTriCount);
	const int bvTreeSize = params->buildBvTree ? dtAlignForType<dtOffMeshConnection>(sizeof(dtBVNode)*params->polyCount*2) : 0;
	const int offMeshConsSize = sizeof(dtOffMeshConnection)*storedOffMeshConCount;
	...
	// Pointer must be advanced by dataSize value
	dtAssert(static_cast<int>(d - data) == dataSize);
