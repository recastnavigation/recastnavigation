...
static constexpr uint16_t MESH_NULL_IDX = 0xffff;
...
int compareItemX(const void *va, const void *vb) {
  const auto *const a = static_cast<const BVItem *>(va);
  const auto *const b = static_cast<const BVItem *>(vb);
...
void calcExtends(const BVItem *items, const int /*nitems*/, const int imin, const int imax,
...
int longestAxis(const uint16_t x, const uint16_t y, const uint16_t z) {
  int axis = 0;
  uint16_t maxVal = x;
  if (y > maxVal) {
    axis = 1;
...
void subdivide(BVItem *items, const int nitems, const int imin, const int imax, int &curNode, dtBVNode *nodes) {
  ...
  if (inum == 1) {
    ...
    const int axis = longestAxis(node.bmax[0] - node.bmin[0], node.bmax[1] - node.bmin[1], node.bmax[2] - node.bmin[2]);
...
int createBVTree(const dtNavMeshCreateParams *params, dtBVNode *nodes, int /*nnodes*/) {
  // Build tree
  const float quantFactor = 1 / params->cs;
  auto *const items = static_cast<BVItem *>(dtAlloc(sizeof(BVItem) * params->polyCount, DT_ALLOC_TEMP));
  for (int i = 0; i < params->polyCount; i++) {
    ...
    if (params->detailMeshes) {
      ...
      it.bmin[0] = static_cast<uint16_t>(dtClamp(static_cast<int>((bmin[0] - params->bmin[0]) * quantFactor), 0, 0xffff));
      it.bmin[1] = static_cast<uint16_t>(dtClamp(static_cast<int>((bmin[1] - params->bmin[1]) * quantFactor), 0, 0xffff));
      it.bmin[2] = static_cast<uint16_t>(dtClamp(static_cast<int>((bmin[2] - params->bmin[2]) * quantFactor), 0, 0xffff));

      it.bmax[0] = static_cast<uint16_t>(dtClamp(static_cast<int>((bmax[0] - params->bmin[0]) * quantFactor), 0, 0xffff));
      it.bmax[1] = static_cast<uint16_t>(dtClamp(static_cast<int>((bmax[1] - params->bmin[1]) * quantFactor), 0, 0xffff));
      it.bmax[2] = static_cast<uint16_t>(dtClamp(static_cast<int>((bmax[2] - params->bmin[2]) * quantFactor), 0, 0xffff));
    } else {
      ...
      for (int j = 1; j < params->nvp; ++j) {
        if (p[j] == MESH_NULL_IDX)
          break;
        const uint16_t x = params->verts[p[j] * 3 + 0];
        const uint16_t y = params->verts[p[j] * 3 + 1];
        const uint16_t z = params->verts[p[j] * 3 + 2];
      ...
      // Remap y
      it.bmin[1] = static_cast<uint16_t>(dtMathFloorf(static_cast<float>(it.bmin[1]) * params->ch / params->cs));
      it.bmax[1] = static_cast<uint16_t>(dtMathCeilf(static_cast<float>(it.bmax[1]) * params->ch / params->cs));
    }
  }
...
uint8_t classifyOffMeshPoint(const float *pt, const float *bmin, const float *bmax) {
  static constexpr uint8_t XP = 1 << 0;
  static constexpr uint8_t ZP = 1 << 1;
  static constexpr uint8_t XM = 1 << 2;
  static constexpr uint8_t ZM = 1 << 3;
...
bool dtCreateNavMeshData(dtNavMeshCreateParams *params, uint8_t **outData, int *outDataSize) {
  ...
  uint8_t *offMeshConClass = nullptr;
  int storedOffMeshConCount = 0;
  int offMeshConLinkCount = 0;

  if (params->offMeshConCount > 0) {
    offMeshConClass = static_cast<uint8_t *>(dtAlloc(sizeof(uint8_t) * params->offMeshConCount * 2, DT_ALLOC_TEMP));
    if (!offMeshConClass)
      return false;
  ...
  auto *data = static_cast<uint8_t *>(dtAlloc(sizeof(uint8_t) * dataSize, DT_ALLOC_PERM));
  if (!data) {
  ...
  for (int i = 0; i < params->offMeshConCount; ++i) {
    ...
    if (offMeshConClass[i * 2 + 0] == 0xff) {
      ...
      p->vertCount = 2;
      p->verts[0] = static_cast<uint16_t>(offMeshVertsBase + n * 2 + 0);
      p->verts[1] = static_cast<uint16_t>(offMeshVertsBase + n * 2 + 1);
  ...
  if (params->detailMeshes) {
    ...
    for (int i = 0; i < params->polyCount; ++i) {
      ...
      dtl.vertCount = static_cast<uint8_t>(ndv - nv);
      dtl.triBase = static_cast<uint32_t>(params->detailMeshes[i * 4 + 2]);
      dtl.triCount = static_cast<uint8_t>(params->detailMeshes[i * 4 + 3]);
      // Copy vertices except the first 'nv' verts which are equal to nav poly verts.
      if (ndv - nv) {
        std::memcpy(&navDVerts[vbase * 3], &params->detailVerts[(vb + nv) * 3], sizeof(float) * 3 * (ndv - nv));
        vbase += static_cast<uint16_t>(ndv - nv);
      }
    }
    // Store triangles.
    std::memcpy(navDTris, params->detailTris, sizeof(uint8_t) * 4 * params->detailTriCount);
  } else {
    ...
    for (int i = 0; i < params->polyCount; ++i) {
      ...
      dtl.triCount = static_cast<uint8_t>(nv - 2);
      // Triangulate polygon (local indices).
      for (int j = 2; j < nv; ++j) {
        uint8_t *t = &navDTris[tbase * 4];
        t[0] = 0;
        t[1] = static_cast<uint8_t>(j - 1);
        t[2] = static_cast<uint8_t>(j);
  ...
  for (int i = 0; i < params->offMeshConCount; ++i) {
    ...
    if (offMeshConClass[i * 2 + 0] == 0xff) {
      ...
      con->poly = static_cast<uint16_t>(offMeshPolyBase + n);
